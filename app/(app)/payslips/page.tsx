    ...
    <>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{period} · {payslips.length} payslips generated</p>
        </div>
          ...
          {canManage && (
            <>
              <button className="btn btn-outline" disabled={draftCount === 0} onClick={markAllSent}>
                <Send size={14} /> Mark All Sent {draftCount > 0 && draftCount}
              </button>
              <button className="btn btn-primary" onClick={() => setShowGen(true)}>
                <SlidersHorizontal size={14} /> Generate Payslips
              </button>
            </>
          )}
        </div>
      ...
      <div className="stat-grid" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
        <div className="card stat"><div className="stat-label">Payslips</div><div className="stat-value">{payslips.length}</div><div className="stat-delta">{sentCount} sent</div></div>
        <div className="card stat"><div className="stat-label">Total deductions</div><div className="stat-value mono" style={{ color: "var(--red)" }}>{money(-totalDeductions)}</div></div>
        <div className="card stat"><div className="stat-label">Net payable</div><div className="stat-value mono">{money(netPayable)}</div><div className="stat-delta">For {period}</div></div>
      </div>

      <div className="card">
        <table className="tbl">
          <thead><tr><th>Employee</th><th>Gross</th><th>Deductions</th><th>Net Pay</th><th>Status</th><th></th></tr></thead>
          <tbody>
            {payslips.map((p) => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td><span className={"pill " + (p.status === "sent" ? "pill-sent" : "pill-draft")}>{p.status === "sent" ? "Sent" : "Draft"}</span></td>
                  <td style={{ textAlign: "right" }}>
                    <button className="btn btn-outline btn-sm" onClick={() => setPreview(p)}><Eye size={13} /> Preview</button>
                  </td>
                </tr>
              );
            })}
            {!loading && payslips.length === 0 && <tr><td colSpan={6} className="empty">No payslips generated for {period} yet.</td></tr>}
          </tbody>
        </table>
      </div>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <p className="modal-title">Generate Payslips</p>
        ...
        <div className="modal-body">
          ...
          <div className="card" style={{ padding: 16 }}>
            <div style={{ fontWeight: 700, fontSize: 13.5, marginBottom: 10 }}>Preview for {period}</div>
            <div style={{ fontSize: 13, color: "var(--muted)" }}>
              Employees included: <span className="mono" style={{ color: "var(--ink)" }}>{activeEmployees.length}</span> active employees.
              Anyone who already has a payslip for this period will be skipped.
            </div>
          </div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-outline" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" disabled={busy} onClick={generate}>{busy ? "Generating…" : "Generate Payslips"}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 480 }} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            <p className="modal-title">Payslip Preview</p>
        ...
        <div className="modal-body">
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: 12, background: "var(--cream)", borderRadius: 10, marginBottom: 16 }}>
            ...
            <span className={"pill " + (sent ? "pill-sent" : "pill-draft")}>{sent ? "Sent" : "Draft"}</span>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <div>
              <div style={{ fontSize: 11, letterSpacing: ".05em", textTransform: "uppercase", color: "var(--green-dark)", fontWeight: 700, marginBottom: 8 }}>Earnings</div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, paddingTop: 8, borderTop: "1px solid var(--border)" }}>
                <span>Gross Pay</span><span className="mono">{money(payslip.gross)}</span>
              </div>
            </div>
            <div>
              <div style={{ fontSize: 11, letterSpacing: ".05em", textTransform: "uppercase", color: "var(--red)", fontWeight: 700, marginBottom: 8 }}>Deductions</div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: 700, paddingTop: 8, borderTop: "1px solid var(--border)" }}>
                <span>Total Deductions</span><span className="mono">-{money(payslip.totalDeductions)}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 18, background: "var(--green)", color: "#fff", borderRadius: 10, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 13 }}>Net Payable</span>
            <span className="mono" style={{ fontSize: 20, fontWeight: 700 }}>{money(payslip.net)}</span>
          </div>
        </div>
        <div className="modal-footer" style={{ justifyContent: "space-between" }}>
          <span style={{ fontSize: 12, color: "var(--muted)" }}>{sent ? "This payslip has been delivered." : "This payslip is still a draft."}</span>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-outline" onClick={() => window.print()}><Printer size={14} /> Print</button>
            {canManage && (
              <button className="btn btn-primary" onClick={onToggleStatus}>{sent ? "Mark as Draft" : "Mark as Sent"}</button>
            )}
          </div>
        </div>
      </div>
